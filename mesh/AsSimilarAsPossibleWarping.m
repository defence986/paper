classdef AsSimilarAsPossibleWarping < handle
    ...
    methods (Access = public)
        ...
        function [warp,Homo] = Warp(obj,Img,gap)
            
            obj.gap = gap;
            Homo = zeros(obj.height,obj.width,3,3);
...
%                    Temp(:,:) = homos(i,j,:,:);
                   Homo(i,j,:,:) = quadWarp(obj,Img,qd1,qd2);
                end
            end
        ...
        function warp = WARP(obj,Img,gap,Homo)
...
%             Homo = zeros(obj.height,obj.width,3,3);
            ...
            for i=1:obj.height-1
                for j=1:obj.width-1
                   ...
                   qd2 = Quad(q0,q1,q2,q3);
                   Temp(:,:) = Homo(i,j,:,:);
                   QuadWarp(obj,Img,qd1,qd2,Temp);

%                    QuadWarp(obj,Img,qd1,qd2,Homo(i,j,:,:));
                end
            end
...
%             rowCount = rowCount + 1;
    ...
    methods (Access = private)%functions of warping
        function H = quadWarp(obj,im,q1,q2)
...
%              obj.warpIm = myWarp(minx,maxx,miny,maxy,im,obj.warpIm,homos,obj.gap);

             obj.warpIm = uint8(obj.warpIm);             
        end 
        function QuadWarp(obj,im,q1,q2,H)
