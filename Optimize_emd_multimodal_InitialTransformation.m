...
for m = 2:size(Homo,2)
    frame2 = read(obj, m);
%     frames{m} = imwarp(frame2,affine2d(Homo{m}),'OutputView',imref2d(size(frame2)));
    frames{m} = imregister(frame2,frame1,'affine',optimizer,metric,...
        'InitialTransformation',affine2d(Homo{m}));
    frame1 = frame2;
end
